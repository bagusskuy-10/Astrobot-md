...
if (!conn.authState.creds.registered) {
  console.log(chalk.bgWhite(chalk.blue('Menunggu soket siap... (10 detik)')))
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      console.error(chalk.bgRed(chalk.white('Gagal meminta kode pairing:')), error.message)
      console.log(chalk.yellow('Silakan restart bot atau periksa apakah nomor sudah benar.'))
    }
  }, 10000)
}
...
conn.newsletterFollow("120363422222410562@newsletter")
            conn.newsletterFollow("120363421096003443@newsletter")
            conn.newsletterFollow("120363403673840124@newsletter")
            conn.newsletterFollow("120363423574630866@newsletter")
            conn.newsletterFollow("120363424076187414@newsletter")
            conn.newsletterFollow("120363402167879973@newsletter")
            conn.newsletterFollow("120363404620935034@newsletter")
            conn.newsletterFollow("120@newsletter")
            conn.newsletterFollow("120@newsletter") 
    ...
    for (let id in global.db.data.chats) {
        ...
        if (jam === "21:00") {
            try {
                ...
                await conn.sendMessage(id, { text: '🔒 *Grup ditutup otomatis (21.00)*' })
            } catch {}
        }
        ...
        if (jam === "04:00") {
            try {
                ...
                await conn.sendMessage(id, { text: '🔓 *Grup dibuka otomatis (04.00)*' })
            } catch {}
        }
    }
